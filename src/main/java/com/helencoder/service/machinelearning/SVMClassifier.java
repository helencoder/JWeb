...
public class SVMClassifier implements Serializable {
    ...
    public String run(String content) throws FileNotFoundException {
        ...
        if (!modelFile.exists()) {
            InputStream is = this.getClass().getResourceAsStream("/static/model/SVMModel");
        ...
        if (modelFile.exists()) {
            // 存在即使用
            model = sparkModelDao.getSVMModel();
        } else {
            // 不存在重新进行归集训练
            ...
            model = SVMWithSGD.train(trainData.rdd(), numIterations);
